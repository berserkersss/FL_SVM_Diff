...
for k in range(len(num_img)):
    #  导入unbalance数据集
    csv_path_train_data = 'user_csv/index_D/' + 'user' + str(k) + 'train_index' + '_unbalance' + '.csv'
    ...
    #  导入balance数据集
    csv_path_train_data = 'user_csv/index_D/' + 'user' + str(k) + 'train_index' + '_balance' + '.csv'
    train_index = pd.read_csv(csv_path_train_data, header=None)
...
X_train_L, y_train_L, X_train_S, y_train_S = [], [], [], []
for k in range(num_device):
    X_train_S.append(X_all_train[dict_users[k], :])
    y_train_S.append(y_all_train[dict_users[k]])
    X_train_L.append(X_all_train[dict_users_balance[k], :])
    y_train_L.append(y_all_train[dict_users_balance[k]])
...
Ws_global_Fvg_L = np.random.rand(N_class, N_features) * 0.1
Ws_local_Fvg_L = [Ws_global_Fvg_L for k in range(num_device)]

Ws_global_Fvg_S = Ws_global_Fvg_L
Ws_local_Fvg_S = [Ws_global_Fvg_S for k in range(num_device)]

Ws_global_Fvg_Optimize_L = Ws_global_Fvg_L
Ws_local_Fvg_Optimize_L = [Ws_global_Fvg_L for k in range(num_device)]

Ws_global_Fvg_Optimize_S = Ws_global_Fvg_L
Ws_local_Fvg_Optimize_S = [Ws_global_Fvg_S for k in range(num_device)]

W_CL = Ws_global_Fvg_L
...
# In[150]:
# 新建存放数据的文件, 并存放初始数据
filename = 'result/SVM_D/' + "Accuracy_CL_iid_SVM_D.csv"
with open(filename, "w") as myfile:
    myfile.write(str(0) + ',')
filename = 'result/SVM_D/' + "Accuracy_FedAvg_S_SVM_D.csv"
with open(filename, "w") as myfile:
    myfile.write(str(0) + ',')
filename = 'result/SVM_D/' + "Accuracy_FedAvg_Optimize_S_SVM_D.csv"
with open(filename, "w") as myfile:
    myfile.write(str(0) + ',')
filename = 'result/SVM_D/' + "Accuracy_FedAvg_L_SVM_D.csv"
with open(filename, "w") as myfile:
    myfile.write(str(0) + ',')
filename = 'result/SVM_D/' + "Accuracy_FedAvg_Optimize_L_SVM_D.csv"
with open(filename, "w") as myfile:
    myfile.write(str(0) + ',')

filename = 'result/SVM_D/' + "Difference_FedAvg_S_SVM_D.csv"
with open(filename, "w") as myfile:
    myfile.write(str(0) + ',')
filename = 'result/SVM_D/' + "Difference_FedAvg_Optimize_S_SVM_D.csv"
with open(filename, "w") as myfile:
    myfile.write(str(0) + ',')
filename = 'result/SVM_D/' + "Difference_FedAvg_L_SVM_D.csv"
with open(filename, "w") as myfile:
    myfile.write(str(0) + ',')
filename = 'result/SVM_D/' + "Difference_FedAvg_Optimize_L_SVM_D.csv"
with open(filename, "w") as myfile:
    myfile.write(str(0) + ',')
...
for t in range(T):
    if T > 0:
        for k in range(num_device):
            # 更新FL的w
            Ws_local_Fvg_S[k] = update_weight(Ws_local_Fvg_S[k], X_train_S[k], y_train_S[k], lam,
                                              eta)

            Ws_local_Fvg_L[k] = update_weight(Ws_local_Fvg_L[k], X_train_L[k], y_train_L[k], lam,
                                              eta)

            Ws_local_Fvg_Optimize_L[k] = update_weight(Ws_local_Fvg_Optimize_L[k], X_train_L[k], y_train_L[k], lam,
                                                       eta)

            Ws_local_Fvg_Optimize_S[k] = update_weight(Ws_local_Fvg_Optimize_S[k], X_train_S[k], y_train_S[k], lam,
                                                       eta)
        # 更新CL的w
        ...
        if t % tau == 0:
            ...
            y_predict = predict(Ws_global_Fvg_S, X_test)
            accuracy = metrics.accuracy_score(y_test, y_predict)
            print("FL: %f" % accuracy)
            ...
            y_predict = predict(Ws_global_Fvg_Optimize_S, X_test)
            accuracy = metrics.accuracy_score(y_test, y_predict)
            ...
            y_predict = predict(Ws_global_Fvg_L, X_test)
            accuracy = metrics.accuracy_score(y_test, y_predict)
            print("FL_L: %f" % accuracy)
            acc_train_fl_his2.append(accuracy)
            filename = 'result/SVM_D/' + "Accuracy_FedAvg_L_SVM_D.csv"
            with open(filename, "a") as myfile:
                myfile.write(str(accuracy) + ',')

            y_predict = predict(Ws_global_Fvg_Optimize_L, X_test)
            accuracy = metrics.accuracy_score(y_test, y_predict)
            print("FL_Optimize_L: %f" % accuracy)
            ...
            #Difference
            Diff = np.linalg.norm(W_CL - Ws_global_Fvg_S)
            diff_train_fl_his.append(Diff)
            filename = 'result/SVM_D/' + "Difference_FedAvg_S_SVM_D.csv"
            with open(filename, "a") as myfile:
                myfile.write(str(Diff) + ',')

            Diff = np.linalg.norm(W_CL - Ws_global_Fvg_Optimize_S)
            diff_train_cl_his.append(Diff)
            filename = 'result/SVM_D/' + "Difference_FedAvg_Optimize_S_SVM_D.csv"
            with open(filename, "a") as myfile:
                myfile.write(str(Diff) + ',')

            Diff = np.linalg.norm(W_CL - Ws_global_Fvg_L)
            diff_train_fl_his2.append(Diff)
            filename = 'result/SVM_D/' + "Difference_FedAvg_L_SVM_D.csv"
            with open(filename, "a") as myfile:
                myfile.write(str(Diff) + ',')

            Diff = np.linalg.norm(W_CL - Ws_global_Fvg_Optimize_L)
            ...
            # Fvg算法
            Ws_global_Fvg_L, Ws_local_Fvg_L = limited_global(X_train_L, Ws_local_Fvg_L)
            Ws_global_Fvg_S, Ws_local_Fvg_S = limited_global(X_train_S, Ws_local_Fvg_S)
            Ws_global_Fvg_Optimize_L, Ws_local_Fvg_Optimize_L = limited_global_optimize(Ld_L, Ws_local_Fvg_Optimize_L)
            Ws_global_Fvg_Optimize_S, Ws_local_Fvg_Optimize_S = limited_global_optimize(Ld, Ws_local_Fvg_Optimize_S)
...
ax.plot(acc_train_fl_his, c=colors[0], label=labels[0], linestyle=linestyles[1])
ax.plot(acc_train_cl_his, c=colors[1], label=labels[1], linestyle=linestyles[2])
ax.plot(acc_train_fl_his2, c=colors[2], label=labels[2], linestyle=linestyles[3])
ax.plot(acc_train_cl_his2, c=colors[3], label=labels[3], linestyle=linestyles[4])
ax.plot(acc_train_cl_his_iid, c=colors[4], label=labels[4], linestyle=linestyles[0])
ax.legend()
plt.xlabel('Iterations')
plt.ylabel('Accuracy')
plt.savefig('Figures/Accuracy_SVM_D.png')
plt.savefig('Figures/Accuracy_SVM_D.eps')
...
fig = plt.figure()
ax = fig.add_subplot(111)
ax.plot(diff_train_fl_his, c=colors[0], label=labels[0], linestyle=linestyles[1])
ax.plot(diff_train_cl_his, c=colors[1], label=labels[1], linestyle=linestyles[2])
ax.plot(diff_train_fl_his2, c=colors[2], label=labels[2], linestyle=linestyles[3])
ax.plot(diff_train_cl_his2, c=colors[3], label=labels[3], linestyle=linestyles[4])
ax.legend()
plt.xlabel('Iterations')
plt.ylabel('Difference')
plt.savefig('Figures/Difference_SVM_D.png')
plt.savefig('Figures/Difference_SVM_D.eps')