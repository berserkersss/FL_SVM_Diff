...
if __name__ == '__main__':
    ...
    for i in range(num):
        ...
        csv_path_accuracy = 'result/SVM2/' + 'Accuracy_' + labels[i] + '_SVM.csv'
        accuracy2 = pd.read_csv(csv_path_accuracy, header=None)
        accuracy2 = accuracy2.values
        ax.plot(accuracy[0]/2+accuracy2[0]/2, c=colors[i], label=labels[i], linestyle=linestyles[i])
        ax.legend()
        plt.xlabel('Iterations')
        plt.ylabel('Accuracy')
    ...
    for i in range(num):
        ...
        if i == 1 or i == 3:
            ...
            accuracy = np.hstack((accuracy, A_array))
        x = np.arange(int((len(accuracy[0])) / 2)) * 2
        ax.plot(x, accuracy[0][x], c=colors[i], label=labels[i], linestyle=linestyles[i])
        ax.legend()
        plt.xlabel('Iterations')
        plt.ylabel('Accuracy')
